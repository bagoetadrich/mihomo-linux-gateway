...
is_bad_server() {
	...
	*:*)
		# IPv6 字面量(含 "[2001:db8::1]" 这种写法), 以及被误写成 "host:port" 的值。
		#
		# 本项目面向**纯 IPv4** 场景: IKEv2 隧道、规则、DNS 都只跑 v4。
		# 混进 v6 节点只会在 url-test 里永远红着, 还容易在面板上被误选,
		# 所以默认丢掉。服务器确实有可用 IPv6 时, 设 ALLOW_IPV6=1 放开。
		if [ "${ALLOW_IPV6:-0}" = "1" ]; then return 1; fi
		return 0
		;;
	esac
	return 1
